...
class GrassFarmBot:
    ...
    async def start(self):
        await self.db.initialize()  # <button class="citation-flag" data-index="5">
    ...
    async def add_account(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user_id = update.effective_user.id  # <button class="citation-flag" data-index="9">
        args = " ".join(context.args).strip()
        if not args:
            await update.message.reply_text("Please provide account details:")
            return
    ...
    async def list_accounts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not accounts:
            ...
            return
        message = "\n".join([
            f"📧 {a['email']} | 🟢 Active" for a in accounts
        ])
    ...
    async def add_proxy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("Proxies added!")
